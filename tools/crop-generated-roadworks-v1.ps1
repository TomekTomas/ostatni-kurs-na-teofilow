param(
  [string]$SheetPath = "assets\generated\roadwork-sprite-sheet-v1.png"
)
...
function Has-TransparentNeighbor($bmp, $x, $y) {
  for ($oy = -1; $oy -le 1; $oy++) {
    for ($ox = -1; $ox -le 1; $ox++) {
      ...
      $nx = $x + $ox
      $ny = $y + $oy
      if ($nx -lt 0 -or $ny -lt 0 -or $nx -ge $bmp.Width -or $ny -ge $bmp.Height) { return $true }
      if ($bmp.GetPixel($nx, $ny).A -eq 0) { return $true }
    }
  }
  return $false
}
  ...
  for ($pass = 0; $pass -lt 2; $pass++) {
    ...
    for ($y = 0; $y -lt $bmp.Height; $y++) {
      for ($x = 0; $x -lt $bmp.Width; $x++) {
        ...
        $isDarkEdgeSpill = $c.G -gt 42 -and $c.R -lt 95 -and $c.B -lt 95 -and $c.G -gt ($c.R * 1.35) -and $c.G -gt ($c.B * 1.35) -and (Has-TransparentNeighbor $bmp $x $y)
        $isGreenResidue = $c.G -gt 62 -and $c.R -lt 120 -and $c.B -lt 110 -and $c.G -gt ($c.R + 16) -and $c.G -gt ($c.B + 20)
        if ($isNeon -or $isEdgeSpill -or $isDarkEdgeSpill -or $isGreenResidue) { $remove.Add("$x,$y") }
      }
    }
    foreach ($p in $remove) {
      $parts = $p.Split(",")
...
function Get-ContentBounds($bmp) {
  ...
  if ($maxX -lt 0) { return [Drawing.Rectangle]::new(0, 0, $bmp.Width, $bmp.Height) }
...
try {
  Save-RectSprite $sheet "prop-road-cone.png" 80 128 260 330 0 132
  Save-RectSprite $sheet "prop-road-cones.png" 395 172 315 275 205
  Save-RectSprite $sheet "road-pothole.png" 748 190 340 285 210
  Save-RectSprite $sheet "prop-road-barrier.png" 1160 205 330 245 205
  Save-RectSprite $sheet "prop-roadwork-sign.png" 90 510 250 325 0 156
  Save-RectSprite $sheet "prop-construction-barrel.png" 435 500 210 340 0 136
  Save-RectSprite $sheet "prop-sandbags.png" 710 585 330 250 180
  Save-RectSprite $sheet "prop-manhole-cover.png" 1135 550 345 285 190
} finally {
  $sheet.Dispose()
}
