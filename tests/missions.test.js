...
describe("cele misji", () => {
  ...
  it("Dojedz do Teofilowa nie przechodzi po przerwaniu kursu", () => {
    const result = missionByLabel(missionResults({ interrupted: true, stats: perfectStats, smoothness: 100, satisfaction: 100, punctuality: 100 }), "Dojedz do Teofilowa");
    expect(result.ok).toBe(false);
  });

  it("Obsluz wszystkie przystanki nie przechodzi przy pominietym stopie", () => {
    const result = missionByLabel(missionResults({ stats: { ...perfectStats, missedStops: 1 }, smoothness: 100, satisfaction: 100, punctuality: 100 }), "Obsluz wszystkie przystanki");
    expect(result.ok).toBe(false);
  });

  it("Plynnosc minimum 75% przechodzi na granicy 75", () => {
    const result = missionByLabel(missionResults({ stats: perfectStats, smoothness: 75, satisfaction: 100, punctuality: 100 }), "Plynnosc minimum 75%");
    expect(result.ok).toBe(true);
  });

  it("Plynnosc minimum 75% nie przechodzi przy 74", () => {
    const result = missionByLabel(missionResults({ stats: perfectStats, smoothness: 74, satisfaction: 100, punctuality: 100 }), "Plynnosc minimum 75%");
    expect(result.ok).toBe(false);
  });

  it("Punktualnosc minimum 70% przechodzi na granicy 70", () => {
    const result = missionByLabel(missionResults({ stats: perfectStats, smoothness: 100, satisfaction: 100, punctuality: 70 }), "Punktualnosc minimum 70%");
    expect(result.ok).toBe(true);
  });

  it("Punktualnosc minimum 70% nie przechodzi przy 69", () => {
    const result = missionByLabel(missionResults({ stats: perfectStats, smoothness: 100, satisfaction: 100, punctuality: 69 }), "Punktualnosc minimum 70%");
    expect(result.ok).toBe(false);
  });
  ...
  it("Zwrotnice bez pomylki nie przechodzi po bledzie zwrotnicy", () => {
    const result = missionByLabel(missionResults({ stats: { ...perfectStats, switchWrong: 1 }, smoothness: 100, satisfaction: 100, punctuality: 100 }), "Zwrotnice bez pomylki");
    expect(result.ok).toBe(false);
  });

  it("Zwrotnice bez pomylki wymaga zaliczenia wszystkich zwrotnic", () => {
    const result = missionByLabel(missionResults({ stats: { ...perfectStats, switchCorrect: SWITCHES.length - 1 }, smoothness: 100, satisfaction: 100, punctuality: 100 }), "Zwrotnice bez pomylki");
    expect(result.ok).toBe(false);
  });
});
